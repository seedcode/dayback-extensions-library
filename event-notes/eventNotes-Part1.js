...
try {
	...
	// name that is applicable to that calendar:
	//
	...
	// The following declares commonly used variables which can be used
	// to restrict the events that are eligible for notes
	...
	function addNotesIcons() {
		...
		if (typeof inputs.includeEventIfTrue == 'function') {
			events = clientEvents.filter((event) => {
				return (
					inputs.includeEventIfTrue(event) &&
					getNotesFieldId(event) !== undefined
				);
			});
		} else {
			events = clientEvents.filter((event) => {
		...
		events.forEach((event) => {
			...
			if (inputs.isHorizonView) {
				...
				if (horizonpills) pills.push(...horizonpills);
			} else {
				if (regularpills) pills.push(...regularpills);

				if (micropills) pills.push(...micropills);
			}
			...
			pills.forEach((e) => {
				...
				div.addEventListener('mouseenter', function () {
					showNotesTooltip('hover', div);
				});
				div.addEventListener('mouseleave', function () {
					setTimeout(function () {
						seedcodeCalendar.init('disableEventClick', undefined);
					}, 150);
				});
				...
				function paintNotesIcon(event) {
					...
					if (
						...
						i.style.setProperty(
							'--background',
							inputs.iconColorWithNote
						);
						i.style.filter = 'brightness(150%) !important';
					} else if (
						inputs.isHorizonView &&
						inputs.horizonIconColorWithoutNote != ''
					) {
						i.style.setProperty(
							'--background',
							inputs.horizonIconColorWithoutNote
				...
				function showNotesTooltip(trigger, div) {
					...
					function postRenderFunction() {
						...
						function registerClickHandlers() {
							let tooltipContainer = document.querySelector(
								'.tooltip.tooltip-custom'
							);

							let clickable = document.querySelector(
						...
						function buildEditRow() {
							...
							tooltipContainer.removeEventListener(
								'click',
								buildEditRow
							);
							clickable.innerText = '';
							...
							deleteButton.onclick = function () {
								...
								updateEvent(events[0], '', clickable);
							};
