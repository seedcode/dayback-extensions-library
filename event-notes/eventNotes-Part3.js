...
function run() {
    ...
    let editNotes = document.querySelector(".tooltip.tooltip-custom .editNotes");
    ...
    function repaintEventPopover() { 
        ...
        function checkPopoup() {
            ...
            let customFields = document.querySelector('.edit-container [content="Custom Fields"]>span[ng-bind="content"]');
            if (customFields) {    
                customFields.innerHTML = '<i class="fa fa-comment notesChatIconIcon"></i> ' + customFields.innerHTML;
                retries = 0;
            } else if (retries > 0) {
                retries--;
