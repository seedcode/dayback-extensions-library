...
function run() {
	...
	function buildResourceList(resources) {
		...
		resources
			...
			.forEach(function (resource) {
				...
				if (
					editEvent.resource &&
					editEvent.resource.includes(resource.name)
				) {
	...
	function addResourceClickAction() {
		var resourceButton = document.querySelector(
			inputs.resourceButtonSelector
		);
		if (resourceButton) {
			retries = 0;
	...
	function addFilterButton() {
		...
		if (nanoContent) {
			...
			clearSelectionDiv.parentNode.insertBefore(
				container,
				clearSelectionDiv
			);
