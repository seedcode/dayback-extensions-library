...
try {
	...
	// There are no variables to configure. This action can run in any source
...
function run() {
	var now = moment();
	var isBefore = event.start.isBefore(now);
	if (isBefore) {
		utilities.showModal(
			'Warning',
			'This appointment starts in the past. Do you want to create it anyway?',
			'cancel',
			cancelFunction,
			'continue',
			confirmFunction
