...
function run() {
    ...
    if (inputs.useResourceTag && inputs.resourceTagGroup != '') {
        ...
        var tagMatch = new RegExp('^' + inputs.resourceTagGroup + ':(.*?)$');

        let resources = globals.seedcodeCalendar.get('resources');
        let foundResource = resources?.find(item => item.name == inputs.accountName);
        let resourceTags = typeof foundResource === 'object' && Array.isArray(foundResource.tags) ? foundResource.tags : [];
        let cssTagDirective = resourceTags.filter(item => item.name.match(tagMatch));
        ...
        if (cssTagDirective.length) {
            ...
            cssTagDirective.forEach(tag => {
                ...
                if (match && match.length && match[1].length) {
                    tagClassSelector = inputs.resourceTagGroup + '-' + match[1].replace(/ /g, '-').replace(/[^A-Z0-9\-]/ig, '');
    ...
    if (inputs.useAccountNameClasses) {
        accountClassSelector = 'account-' + inputs.accountName.replace(/ /g, '-').replace(/[^A-Z0-9\-]/ig, '');
